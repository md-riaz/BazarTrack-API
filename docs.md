...
- **Record expenses or advances** – `POST /api/payments` with `type` set to `debit` or `credit`; assistants must include `owner_id` to associate the entry.
...
### Wallet & Payments
- **Advance/expense entries** – A payment entry (credit or debit) is recorded through POST /api/payments; credits (advances) are created by owners, while assistants include `owner_id` when logging debits for expenses
...
- `owner_id` – id of the owner associated with the payment (required for `debit` entries)
